document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    if (data.university === null || data.university === undefined) {
      ...
      const universities = await fetch("/account/get_unis").then((res) =>
        res.json()
      );
      ...
      requestBtn.onclick = async () => {
        const universityId = parseInt(dropdown.value);
        const token = localStorage.getItem("token");
        ...
        try {
          const res = await fetch(
            `/account/add_uni_user?token=${token}&uni_id=${universityId}`,
            {
              method: "POST",
            }
          );
    ...
    document.getElementById("user-university").textContent =
    ...
    if (data.role === "admin") {
      ...
      const response = await fetch(
        `/account/get_uni_requests?token=${token}&uni_id=${data.university_id}`
      );
      ...
      if (pendingRequests.length === 0) {
        ...
        pendingRequests.forEach((request) => {
          ...
          acceptButton.onclick = () =>
            handleAcceptRequest(request.id, data.university_id);
...
function handleAcceptRequest(user_id, university_id) {
  const token = localStorage.getItem("token");
  fetch(
    `/account/accept_uni_request?token=${token}&user_id=${user_id}&university_id=${university_id}`,
    {
      method: "POST",
    }
  )
